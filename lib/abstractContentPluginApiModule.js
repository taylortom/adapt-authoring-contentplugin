...
class AbstractContentPluginApiModule extends AbstractApiModule {
  ...
  async uninstallPlugin(id) {
    ...
    const [ pluginData ] = await mongodb.find(...params);
  ...
  async checkCompatibility({ name, version, framework, targetAttribute }) {
    const mongodb = await this.app.waitForModule('mongodb');
    const [ installedPlugin ] = await mongodb.find(this.collectionName, { name });

    if (installedPlugin && semver.eq(installedPlugin.version, version)) {
