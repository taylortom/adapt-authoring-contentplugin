...
class ContentPluginModule extends AbstractApiModule {
  ...
  async installLocalPlugins() {
    const localPluginPaths = await globPromise(`${this.framework.path}/src/+(components|extensions|menu|theme)/*`, { absolute: true });
    await Promise.all(localPluginPaths.map(async p => {
      const name = p.split('/').pop();
      try {
        await this.manualInstallPlugin(p, { isZip: false });
        this.log('debug', `installed local plugin '${name}'`);
      } catch(e) {
        if(e.code !== 'EEXIST') this.log('warn', e);
      }
    }));
  }
  ...
  */
  async checkCompatibility({ name, version, framework, targetAttribute }) {
    ...
    if (!semver.satisfies(this.framework.version, framework)) {
      throw InstallError.EINCOMPAT({ name, version });
    }
...
class InstallError extends Error {
  ...
  static EINCOMPAT({ name, version, framework }) {
    return new InstallError(`Plugin ${name}@${version} incompatible with installed framework (${framework})`, 'EINCOMPAT');
  }
