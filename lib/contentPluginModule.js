...
class ContentPluginModule extends AbstractApiModule {
  ...
  async init() {
    try {
      await this.installLocalPlugins();
    } catch(e) {
      this.log('error', e);
    }
    super.init();
  }
  async installLocalPlugins() {
    const installedPlugins = (await this.find({})).reduce((m, { name, version }) => {
      return Object.assign(m, { [name]: version });
    }, {});
    const localPluginPaths = await globPromise('*/*/bower.json', { cwd: path.join(this.framework.path, 'src'), absolute: true });
    const localPluginConfigs = await Promise.all(localPluginPaths.map(async p => {
      return Object.assign(await fs.readJson(p), { dir: p.replace('/bower.json', '') });
    }));
    const newPlugins = localPluginConfigs.filter(({ name, version }) => {
      return !installedPlugins[name] || installedPlugins[name] && semver.gt(version, installedPlugins[name]);
    });
    return Promise.all(newPlugins.map(async p => {
      try {
        await this.checkCompatibility(p);
        await this.insertToDatabase(p);
        this.log('debug', `installed local plugin ${p.name}@${p.version}`);
      } catch(e) {
        this.log('warn', `failed to local plugin ${p.name}@${p.version}, ${e}`);
      }
    }));
  }
  /**
  ...
  */
  addType(pluginData) {
    ...
    const type = Object.keys(pluginData).find(key => ['component', 'extension', 'menu', 'theme'].includes(key));
