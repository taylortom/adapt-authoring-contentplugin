...
class ContentPluginModule extends AbstractApiModule {
  ...
  async installPlugins(plugins, options = { strict: false }) {
    ...
    await Promise.allSettled(plugins.map(async p => {
      const data = await AdaptCli.installPlugins({ plugins: [p], cwd: this.framework.path, logger: console });
  ...
  async uninstallPlugin(_id) {
    ...
    await Promise.allSettled([
      AdaptCli.uninstallPlugins({ plugins: [pluginData.name], cwd: this.framework.path }),
      this.delete({ _id })
    ]);
    return pluginData;
  }
