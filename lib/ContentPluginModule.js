  ...
  async find(query={}, options={}, mongoOptions={}) {
    ...
    if(includeUpdateInfo) {
      ...
      updateInfo.forEach(info => {
        const i = results.findIndex(r => r.name === info.name);
        if(i > -1) {
          results[i].canBeUpdated = info.canBeUpdated;
          results[i].latestCompatibleVersion = info.latestCompatibleSourceVersion;
        }
      });
    }
    return results;
