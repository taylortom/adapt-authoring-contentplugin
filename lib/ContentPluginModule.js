...
class ContentPluginModule extends AbstractApiModule {
  ...
  async installPlugins() {
    ...
    // add any custom plugins as dependencies
    ...
    for(const [name, version] of deps) {
      if(existingPlugins[name]) {
        ...
        try {
          await fs.stat(dest);
        } catch(e) { // local copy is missing, so check the custom plugins cache
          ...
          } // when all else fails, try a fresh install using the CLI
          await this.installPlugin(name, existingVersion);
          this.log('debug', `reinstalled ${name}@${version} successfully`);
        }
        continue;
      }
      try {
