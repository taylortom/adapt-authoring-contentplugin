...
class ContentPluginModule extends AbstractApiModule {
  ...
  async find(query={}, options={}, mongoOptions={}) {
    const includeUpdateInfo = options.includeUpdateInfo === true || query.includeUpdateInfo === 'true';
  ...
  async installPlugins(plugins, options = { strict: false }) {
    const jsonschema = await this.app.waitForModule('jsonschema');
    const errors = []; 
    const existing = (await this.find()).map(p => p.name);
    const installed = [];
    await Promise.allSettled(plugins.map(async p => {
      ...
      await Promise.all(data.map(async d => {
        ...
        try {
          ...
          const func = existing.includes(info.name) ? this.update : this.insert;
          installed.push(await func({ name: info.name }, { 
            ...info,
