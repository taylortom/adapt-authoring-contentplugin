...
class ContentPluginModule extends AbstractApiModule {
  ...
  async initPlugins() {
    ...
    if(!dbPlugins.length) {
      await AdaptCli.installPlugins(cliOpts);
      const pluginData = await AdaptCli.getPluginUpdateInfos(cliOpts);
      await Promise.all(pluginData.map(async p => {
        this.insertToDatabase({ ...await p.getInfo(), type: await p.getType(), isLocalInstall: false });
      }));
      this.log('debug', 'contentplugins initialised from adapt.json');
      return;
    }
    ...
    await Promise.all(dbPlugins.map(async p => {
      ...
      try {
        ...
        await fs.stat(dir);
        s += dir; // local cached copy exists, use that
      } catch(e) {
        s += p.version; // no cached version, try the CLI's DB
      }
      this.log('debug', `reinitialising ${s}`);
      toInstall.push(s);
    }));
    if(!toInstall?.length) {
      return this.log('debug', 'no contentplugins to initialise');
    }
    await AdaptCli.installPlugins({ ...cliOpts, plugins: toInstall });
  ...
  async installHandler(req, res, next) {
    try {
      ...
      const pluginData = await this.installPlugin(name);
  ...
  async installPlugin(plugin, version) {
    const pluginData = await AdaptCli.installPlugins({ 
      plugins: [`${plugin}${version === '*' ? '' : `@${version}`}`], 
      cwd: this.framework.path 
    });
    pluginData.isLocalInstall = pluginData.isLocalSource || pluginData.isLocalZip;
  ...
  async updatePlugin(_id) {
    const [installedPluginData] = await this.find({ _id });
    const pluginData = await AdaptCli.updatePlugins({ plugins: [installedPluginData.name], cwd: this.framework.path });
  ...
  async uninstallPlugin(_id) {
    ...
    return AdaptCli.uninstallPlugins({ plugins: [pluginData.name], cwd: this.framework.path });
  }
  /**
