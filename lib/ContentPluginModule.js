...
/** @ignore */ const globPromise = util.promisify(require('glob'));
/**
...
class ContentPluginModule extends AbstractApiModule {
  ...
  async installPlugins() {
    // build list of dependencies (including any custom plugins)
    const deps = Object.entries((await fs.readJSON(`${this.framework.path}/adapt.json`)).dependencies);
    const pluginsDir = path.resolve(this.app.rootDir, this.getConfig('pluginInstallDir'));
    // make sure the folder exists
    await fs.ensureDir(pluginsDir);

    await Promise.all((await fs.readdir(pluginsDir)).map(async p => {
      const pkg = await fs.readJson(`${pluginsDir}/${p}/bower.json`);
      deps.push([pkg.name, pkg.version, pkg]);
    }));
    const existingPlugins = (await this.find()).reduce((m,p) => Object.assign(m, { [p.name]: p }), {});
    const cliPlugins = [];
    // check all specified plugins are installed in the local framework, if not reinstall them
    for(const [name, version, pkg] of deps) {
      if(!existingPlugins[name]) { // plugin not in DB, mark for cli install
        cliPlugins.push([name, version]);
        continue;
      }
      const { version: existingVersion, type: existingType } = existingPlugins[name];
      const dest = `${this.framework.getPluginPath(existingType)}/${name}`;
      try {
        await fs.stat(dest);
        continue;
      } catch { // local copy is missing, so needs to be reinstalled
        try { // check the custom plugins cache first
          const cachedPath = `${this.getConfig('pluginInstallDir')}/${name}`;
          if(!await fs.pathExists(cachedPath)) {
            throw new Error(`Cached version of ${name} does not exist`);
          }
          try {
            await this.checkCompatibility(pkg);
          } catch(e) {
            if(e.code !== 'EEXIST') this.log('warn', `couldn't install cached plugin, ${e.message}`);
          }
          await fs.copy(cachedPath, dest);
          continue;
        } catch(e) {} // no custom version, install with CLI
        cliPlugins.push([name, existingVersion]);
      }
    }
    for(const [name, version] of cliPlugins) {
      try {
        await this.installPlugin(name, version);
        this.newPlugins.push(name);
        this.log('debug', `successfully installed ${name}@${version}`);
      } catch(e) {
        if(e.code !== 'EEXIST') this.log('warn', `couldn't install ${name}@${version}, ${e.message}`);
      }
    }
  }
  /**
  ...
  async checkCompatibility({ name, version, framework, targetAttribute }) {
    ...
    if(installedPluginData && semver.gt(installedPluginData.version, version)) {
      throw InstallError.ENEWER({ name, version, existingVersion: installedPluginData.version });
    }
    ...
    if(!targetAttribute) {
      throw InstallError.ENOATTR({ name });
    }
...
/**
 * Describes a plugin install error
 * @extends {Error}
 */
class InstallError extends Error {
  /**
   * Creates an error to signify that a plugin already exists
   * @param {Object} data Plugin data
   * @param {String} data.name Name of the plugin
   * @param {String} data.version The version of the plugin
   * @returns {InstallError} The error
   */
  static EEXIST({ name, version }) {
    return new InstallError(`Plugin ${name}@${version} already exists`, 'EEXIST');
  }
  /**
   * Creates an error to signify that a plugin is incompatible with the application
   * @param {Object} data Plugin data
   * @param {String} data.name Name of the plugin
   * @param {String} data.version The version of the plugin
   * @param {String} data.requiredFramework The required version of the framework
   * @param {String} data.installedFramework The version of the framework that's installed
   * @returns {InstallError} The error
   */
  ...
  /**
   * Creates an error to signify that a newer version of the plugin is already installed
   * @param {Object} data Plugin data
   * @param {String} data.name Name of the plugin
   * @param {String} data.version The version of the plugin
   * @param {String} data.existingVersion The version of the currently intalled plugin
   * @returns {InstallError} The error
   */
   static ENEWER({ name, version, existingVersion }) {
    return new InstallError(`Plugin ${name}@${version} already exists at a higher version (${existingVersion})`, 'ENEWER');
  }
  /**
  /**
   * Creates an error to signify that the targetAttribute value is missing
   * @param {Object} data Plugin data
   * @param {String} data.name Name of the plugin
   * @returns {InstallError} The error
   */
   static ENOATTR({ name }) {
    return new InstallError(`Plugin ${name} is missing a targetAttribute value`, 'ENOATTR');
  }
  /**
   * Creates an error to signify that there's a problem with the targetAttribute value
   * @param {Object} data Plugin data
   * @param {String} data.name Name of the plugin
   * @param {String} data.targetAttribute Plugin targetAttribute value
   * @returns {InstallError} The error
   */
  ...
  constructor(message, code) {
    super(message);
    /** @ignore */this.name = 'ContentPluginInstallError';
    /** @ignore */this.code = code;
    /** @ignore */this.statusCode = 400;
  }
}
