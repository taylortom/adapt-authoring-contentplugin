...
class ContentPluginModule extends AbstractApiModule {
    ...
    /** @ignore */ this.routes = [
      {
        route: '/install',
        handlers: { post: [
          middleware.fileUploadParser(['application/zip', 'application/x-compressed-zip'], { unzip: true }),
  ...
  async installPlugins(plugins) {
    return Promise.all(plugins.map(async p => {
      try {
        const [data] = await AdaptCli.installPlugins({ plugins: [p], cwd: this.framework.path });
        await this.update({ name: data.name }, { 
          ...await data.getInfo(),
          type: await data.getType(), 
          isLocalInstall: data.isLocalSource 
        }, { upsert: true }); // update & upsert as not an error if the doc exists
      } catch(e) {
        this.log('warn', `failed to install plugin '${p}', ${e?.data?.error || e}`);
      }
    }));
  }
  /**
