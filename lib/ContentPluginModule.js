...
class ContentPluginModule extends AbstractApiModule {
  ...
  async processPluginSchemas (pluginInfo) {
    ...
    console.log('-------> processPluginSchemas:', JSON.stringify(this.cliArgs, null 2))
    console.log('-------> processPluginSchemas: installed', pluginInfo.map(p => p.name))
    const jsonschema = await this.app.waitForModule('jsonschema')
    return Promise.all(pluginInfo.map(async plugin => {
      const name = plugin.name
      const oldSchemaPaths = this.pluginSchemas[name]
      if (oldSchemaPaths) {
        console.log('processPluginSchemas: old ', name, Object.keys(oldSchemaPaths).length)
      ...
      console.log('-------> processPluginSchemas: new ', name, schemaPaths.length)
