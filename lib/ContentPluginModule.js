...
class ContentPluginModule extends AbstractApiModule {
  ...
  /** @override */
  async parseQuery(schemaName, query, options, mongoOptions) {
    const query = super.parseQuery(schemaName, query, options, mongoOptions);
    // add special-case includeUpdateInfo option from query if it exists
    if(options.includeUpdateInfo) query.includeUpdateInfo = options.includeUpdateInfo === 'true';
    return query;
  }
  /** @override */
  async find(query, options={}, mongoOptions={}) {
    const results = await super.find(query, options, mongoOptions);
    if(options.includeUpdateInfo === true) {
  ...
  async initPlugins() {
    ...
    if(!dbPlugins.length) {
      ...
      await this.installPlugins(plugins);
    ...
    await Promise.allSettled(plugins.map(async p => {
      ...
      await Promise.all(data.map(async d => {
        ...
        try {
          ...
          installed.push(await this.update({ name: info.name }, { 
            ...info,
          ...
          const schemas = await globPromise(`src/*/${info.name}/schema/*.schema.json`, { cwd: this.framework.path, absolute: true });
          await Promise.allSettled(schemas.map(async f => jsonschema.registerSchema(f)));
          this.log('info', `successfully installed plugin '${info.name}@${info.version}'`);
        } catch(e) {
          this.log('warn', `failed to install plugin ${e.data.name}, ${e?.data?.error || e}`);
          errors.push(e);
        }
      }));
    }));
  ...
  serveSchema() {
    return async (req, res, next) => {
      try {
        ...
        if(!schema) {
          return res.sendError(this.app.errors.NOT_FOUND.setData({ type: 'schema' }));
        }
   ...
   */
   async installHandler(req, res, next) {
    try {
      ...
      if(!pluginData) {
        return res.sendError(this.app.errors.CONTENTPLUGIN_INVALID_ZIP);
      }
