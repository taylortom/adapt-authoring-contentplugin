...
class ContentPluginModule extends AbstractApiModule {
  ...
  async installPlugin(plugin, version) {
    ...
    return this.insertToDatabase({ ...pluginData, isLocalInstall: pluginData.isLocalSource });
  }
  /**
  ...
  async insertToDatabase(pluginData) {
    ...
    return existingData ? this.replace({ name: pluginData.name }, pluginData) : this.insert(pluginData);
  }
}
