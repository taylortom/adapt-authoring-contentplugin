...
class ContentPluginModule extends AbstractApiModule {
  ...
  async installPlugins() {
    ...
    await Promise.all((await fs.readdir(pluginsDir)).map(async p => {
      deps.push(await fs.readJson(`${pluginsDir}/${p}/bower.json`));
    }));
    ...
    for(const d of deps) {
      ...
      try {
        ...
        try { // check the custom plugins cache first
          ...
          await fs.pathExists(cachedPath);
          try {
            this.checkCompatibility(d);
          } catch(e) {
            this.log('warn', `Framework plugin '${name}' is no longer compatible with the installed framework version`);
          }
