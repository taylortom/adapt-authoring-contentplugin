...
import { api as AdaptCli } from 'adapt-cli';
 ...
 */
export default class ContentPluginModule extends AbstractApiModule {
  /** @override */
  async setValues() {
  ...
  async installPlugins() {
    ...
    await Promise.all(Object.values(dbPlugins).filter(p => p.isLocalInstall).map(async p => {
      ...
      try {
        ...
        if(e.code !== 'EEXIST') this.log('error', `couldn't reinstall locally installed plugin ${p.name}, ${e.message}`);
      }
    }));
  ...
  async runCliTask(task, plugin) {
    return AdaptCli.commands[task](plugin, this.framework.path);
  }
  /**
