    ...
    return Promise.all(pluginInfo.map(async plugin => {
      ...
      return Promise.all(schemaPaths.map(async schemaPath => {
        ...
        if(source) {
          ...
          if(this.pluginSchemas[name][source]) jsonschema.deregisterSchema(this.pluginSchemas[name][source]);
          this.pluginSchemas[name][source] = schema.$anchor;
        }
    ...
    await Promise.allSettled(plugins.map(async p => {
      ...
      await Promise.all(data.map(async d => {
        ...
        try {
          ...
          await this.processPluginSchemas([d]);
