...
class ContentPluginModule extends AbstractApiModule {
  ...
  async init() {
     ...
     * @type {String}
     */
    this.locaPluginsDir = this.getConfig('pluginInstallDir');
    try {
   ...
   * @param {*} data Data to be sent to the DB
   * @param {*} options Options to pass to the DB function 
  ...
  async initPlugins() {
    ...
    if(!dbPlugins.length) {
      const plugins = Object.entries(this.framework.plugindata.dependencies).map(([name, version]) => `${name}#${version}`);
      return this.installPlugins(plugins);
    }
    const installedPlugins = (await AdaptCli.getPluginUpdateInfos(this.cliArgs)).reduce((m, p) => Object.assign(m, { [p.name]: p }), {});
    const toInstall = [];
  ...
  async processPluginSchemas() {
    ...
    const pluginInfo = await AdaptCli.getPluginUpdateInfos(this.cliArgs);
    return Promise.all(pluginInfo.map(async plugin => {
      return Promise.all((await plugin.getSchemaPaths()).map(async p => {
        const name = plugin.name;
        const s = await fs.readJSON(p);
        const source = s?.$patch?.source?.$ref;
        if(source) {
          if(!this.pluginSchemas[name]) this.pluginSchemas[name] = {};
          if(!this.pluginSchemas[name][source]) this.pluginSchemas[name][source] = s.$anchor;
        }
  ...
  /**
   * Returns plugin's extension schema for the specified source schema
   * @param {String} pluginName 
   * @param {String} sourceSchema 
   * @return {Promise} Resolves with the loaded schema
   */
  async getPluginExtensionSchema(pluginName, sourceSchema) {
    const jsonschema = await this.app.waitForModule('jsonschema');
    const schemaName = this.pluginSchemas[pluginName] && this.pluginSchemas[pluginName][sourceSchema];
    if(schemaName) return jsonschema.getSchema(schemaName, { applyExtensions: false });
  }
  /**
    ...
    await Promise.allSettled(plugins.map(async p => {
      ...
      await Promise.all(data.map(async d => {
        ...
        try {
          ...
          installed.push(await this.insertOrUpdate({
            ...info, 
            type: await d.getType(), 
            isLocalInstall: d.isLocalSource
          }));
          // register any new schemas
          const schemaPaths = await globPromise(`src/*/${info.name}/schema/*.schema.json`, { cwd: this.framework.path, absolute: true });
          await Promise.allSettled(schemaPaths.map(async s => jsonschema.registerSchema(s, { replace: true })));
          this.log('info', 'PLUGIN_INSTALL', `${info.name}@${info.version}`);
        } catch(e) {
          this.log('warn', 'PLUGIN_INSTALL_FAIL', e.data.name, e?.data?.error || e);
          errors.push(e);
        }
      }));
    }));
   ...
   */
   async installHandler(req, res, next) {
    try {
      let name = req.body.name || req.fileUpload.files.file[0].filepath;
      const contents = await fs.readdir(name);
      if(contents.length === 1) {
        name += path.sep + contents[0];
      }
      const bowerData = JSON.parse(await fs.readFile(path.join(name, 'bower.json')));
      const [existing] = await this.find({ name: bowerData.name });

      if(existing) {
        if(semver.lte(bowerData.version, existing.version)) {
          throw this.app.errors.CONTENTPLUGIN_ALREADY_EXISTS
            .setData({ name: existing.name, version: existing.version });
        }
        if(!req.auth.isSuper && !req.auth.scopes.includes('replacecliinstall:contentplugin')) {
